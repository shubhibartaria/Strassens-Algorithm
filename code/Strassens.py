...
'''
This program will execute the Strassens Algorithm for Matrix Multiplication with 1 approach: with leaf size=2
'''
def main():
...
class Strassenexec:
	...
	def matMultiply(A,B,n):
		'''
		This method multiplies matrix of size n(here it is 2) in normal matrix multiplication way
		'''
		C=[[0 for x in range(n)] for y in range(n)]
		for i in range(n):
			for j in range(n):
